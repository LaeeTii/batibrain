...
| V1-04 | Murs et topologie | Terminé | Murs autonomes, sommets partagés, suppression d'un sommet avec raccordement de ses voisins et cardinalité maximale de deux pièces garanties dans le domaine et la base. |
...
| V1-21 | Canvas partagé | À valider | Canvas2D est partagé par les éditeurs global et pièce, avec couleur de sol par pièce, pictogrammes métier, menu contextuel des sommets et cadrage initial centré sur l'emprise géométrique; les autres niveaux visibles de l’éditeur global et les autres pièces de l’éditeur par pièce partagent un filigrane pointillé non interactif, superposé selon l’altitude pour les niveaux; WallElevationCanvas reprend React-Konva, le zoom et l’échelle du socle commun. |
| V1-22 | Sélection, panneaux, historique | À valider | Sélection synchronisée, fermeture des contextes par clic simple sur le fond, conservation pendant le panoramique et historique de vingt actions sont actifs; les blocs d'édition exposent la coupe et le détachement réel d'un mur, le sens et le côté ouvrant, le repositionnement de cote et la réassociation de note via des modes canvas annulables avec Echap. Le brouillon canonique est recalculé dès la création d'une pièce, ce qui autorise le détachement avant toute sauvegarde; détacher une extrémité ouvre le contour, supprime la pièce concernée et conserve ses murs autonomes et leurs sommets déplaçables. Le magnétisme des sommets permet de raccorder ces murs et recrée une pièce lorsqu'un cycle simple est refermé. Les sauvegardes successives nettoient puis réécrivent les murs autonomes sans collision d'identifiants, et leur relecture conserve toute la géométrie autonome dans le brouillon et le canvas. Le sélecteur de magnétisme adjacent à Affichage pilote six sources et la distance persistées par utilisateur-projet. `npm run check` est vert avec 43 fichiers et 223 tests; la suite SQL est verte avec 105 tests. Recette UI complète à exécuter. |
...
| V1-26 | RoomEditor2DView | À valider | Canvas2D partagé avec cadrage limité à la pièce courante et affichage optionnel des autres pièces en filigrane non interactif, contexte strict de pièce, brouillon canonique complet du niveau, sauvegarde transactionnelle, historique, droits et navigation vers WallEditorView sont actifs. |

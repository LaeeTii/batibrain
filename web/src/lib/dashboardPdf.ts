...
function addRoomPage(pdf: jsPDF, snapshot: RoomSnapshot, level: Level | undefined, projectName: string, mode: RoomPdfMode) {
  ...
  if (mode === 'détail') {
    ...
    pdf.setFontSize(10);
    pdf.text(`Surface : ${getRoomAreaM2(snapshot.vertices).toLocaleString('fr-FR', { maximumFractionDigits: 2 })} m²`, 20, y + 10);
    pdf.text(`Périmètre : ${polygonPerimeterCm(snapshot.vertices).toLocaleString('fr-FR', { maximumFractionDigits: 1 })} cm`, 20, y + 18);
...
export function exportDashboardPdf(projectName: string, levels: Level[], snapshots: RoomSnapshot[], mode: RoomPdfMode) {
  if (snapshots.length === 0) throw new Error('Aucune pièce visible à exporter.');
  const pdf = new jsPDF({ format: 'a4', unit: 'mm' });
  snapshots.forEach((snapshot, index) => {
    if (index > 0) pdf.addPage();
    addRoomPage(pdf, snapshot, levels.find(({ id }) => id === snapshot.room.levelId), projectName, mode);
  });

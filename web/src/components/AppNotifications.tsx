...
export function AppNotifications({ onProjectAccepted, gateway = supabaseCollaborationGateway }: { onProjectAccepted(projectId: string): void; gateway?: CollaborationGateway }) {
  ...
  return <div className="app-notificationsHost">
    <button className="app-iconButton" type="button" onClick={() => { setOpen((value) => !value); if (!open) void load(); }} aria-label="Ouvrir les notifications" aria-expanded={open} title="Ouvrir les notifications">
      <LuBell aria-hidden="true" />{count > 0 && <span className="app-notificationsBadge">{count}</span>}
    </button>
    {open && <section className="app-notifications" aria-label="Notifications">
